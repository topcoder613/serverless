...
class AwsCompileS3Events {
  constructor(serverless) {
    ...
    this.provider.needsCustomResource = false;

    this.serverless.service.getAllFunctions()
      .forEach((functionName) => {
        const events = this.serverless.service.getAllEventsInFunction(functionName);
        _.forEach(events, (event) => { // eslint-disable-line consistent-return
          if (event.s3 && _.isObject(event.s3) && event.s3.existing) {
            this.provider.needsCustomResource = true;
          }
        });
      });
    ...
    this.hooks = {
      'before:package:compileFunctions': () => {
        if (this.provider.needsCustomResource) {
          addCustomResourceToService.call(this, 's3');
        }
      },
      ...
      if (functionObj.events) {
        functionObj.events.forEach((event, idx) => {
          if (event.s3 && _.isObject(event.s3) && event.s3.existing) {
            ...
            const customResourceFunctionLogicalId = this.provider.naming.getLambdaLogicalId(
