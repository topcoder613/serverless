...
async function getConfiguration(config) {
  ...
  return s3.getBucketNotificationConfiguration(payload).promise().then((data) => data);
}
...
async function updateConfiguration(config) {
  ...
  NotificationConfiguration.LambdaFunctionConfigurations = NotificationConfiguration
    .LambdaFunctionConfigurations
    .filter(conf => !conf.Id.startsWith(functionName));
